...
const FlightAndHotel = ({ showLoader, inputs, setInputs, setFlightsHotels }) => {
    ...
    const startSearch = async (e) => {
        ...
        e.preventDefault()
        try {
            const response = await axios.get('https://niklas1531-check24.herokuapp.com/findflightshotels', { params: { outbounddepartureairport: inputs.outbounddepartureairport, outboundarrivalairport: inputs.outboundarrivalairport, departuredate:inputs.departuredate, returndate:inputs.returndate, countadults:inputs.countadults, countchildren:inputs.countchildren } })
